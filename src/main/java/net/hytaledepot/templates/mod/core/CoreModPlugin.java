...
public final class CoreModPlugin extends JavaPlugin {
  ...
  private final CoreModTemplate service = new CoreModTemplate();
  private final AtomicLong heartbeatTicks = new AtomicLong();
  private final ScheduledExecutorService scheduler =
      Executors.newSingleThreadScheduledExecutor(
          runnable -> {
            Thread thread = new Thread(runnable, "hd-core-mod-worker");
  ...
  public CoreModPlugin(JavaPluginInit init) {
    super(init);
  }
  ...
  protected void setup() {
    ...
    getCommandRegistry().registerCommand(new CoreModStatusCommand());
    getCommandRegistry().registerCommand(new CoreModDemoCommand());
        ...
        scheduler.scheduleAtFixedRate(
            () -> {
              try {
                ...
                if (tick % 60 == 0) {
                  getLogger().atInfo().log("[CoreMod] heartbeat=%d", tick);
                }
              } catch (Exception exception) {
                lifecycle = Lifecycle.FAILED;
                service.incrementErrorCount();
                getLogger().atInfo().log("[CoreMod] heartbeat failed: %s", exception.getMessage());
              }
            },
            1,
            1,
  ...
  private final class CoreModStatusCommand extends CommandBase {
    private CoreModStatusCommand() {
      super("hdcoremodstatus", "Shows runtime status for CoreModPlugin.");
  ...
  private final class CoreModDemoCommand extends CommandBase {
    private CoreModDemoCommand() {
      super("hdcoremoddemo", "Runs a demo action for CoreModPlugin.");
    setAllowsExtraArguments(true);
      this.setPermissionGroup(GameMode.Adventure);
    }
